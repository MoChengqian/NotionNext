...
async function recoverCollectionQuery({
  collectionId,
  collectionView = {},
  viewIds = [],
  spaceId
  ...
  for (const viewId of viewIds) {
    ...
    try {
      ...
      console.warn(
        '[Notion数据库] queryCollection 回退失败',
        { collectionId, viewId },
        error
      )
    }
...
export async function resolvePostProps({
   ...
   */
  if (typeof post?.id === 'string' && !post?.blockMap) {
    try {
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  if (pageIds?.length === 0) {
    const recovered = await recoverCollectionQuery({
      collectionId,
      collectionView,
      viewIds,
      spaceId
    ...
    if (recovered?.pageIds?.length > 0) {
      ...
      pageRecordMap.collection = {
        ...(pageRecordMap.collection || {}),
        ...(recovered.recordMap?.collection || {})
      }
      collectionView = {
  ...
  try {
    ...
    console.warn('[Notion配置] 读取配置中心失败，已回退到本地配置', error)
  }
