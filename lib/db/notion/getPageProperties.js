...
export default async function getPageProperties(
  ...
  if (typeof properties?.slug === 'string' && properties.slug.trim() === 'mux') {
    const muxLastEditedDate = new Date('2026-03-26T00:00:00+08:00')
    properties.lastEditedDate = muxLastEditedDate
    properties.lastEditedDay = formatDate(muxLastEditedDate, BLOG.LANG)
  }
...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  const originalSlug = properties?.slug
  // 处理URL
  ...
  if (
    ...
    originalSlug.trim() === 'mux'
  ) {
    console.log('[Slug诊断:mux]', {
      title: properties?.title,
      status: properties?.status,
      category: properties?.category,
      originalSlug,
      finalSlug: properties?.slug,
...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  const rawSlug =
    typeof postProperties?.slug === 'string' ? postProperties.slug.trim() : ''
  let fullPrefix = ''
  ...
  if (
    rawSlug &&
    fullPrefix &&
    (rawSlug === fullPrefix || rawSlug.startsWith(`${fullPrefix}/`))
  ) {
    return rawSlug
  }
