...
const extractPageIdsFromCollectionData = result => {
  const ids =
...
async function recoverConfigCollectionQuery({
  collectionId,
  collectionView = {},
  viewIds = [],
  spaceId
  ...
  const recoveredPageIds = new Set()
  const recoveredQuery = {}
  let mergedRecordMap = null

  for (const viewId of viewIds) {
    const viewRecord =
      collectionView?.[viewId]?.value ||
      collectionView?.[viewId] ||
      collectionView?.[normalizeId(viewId)]?.value ||
      collectionView?.[normalizeId(viewId)] ||
      null
    ...
    try {
      ...
      const ids = extractPageIdsFromCollectionData(collectionData)
      ids.forEach(id => recoveredPageIds.add(id))

      if (!recoveredQuery[collectionId]) {
        recoveredQuery[collectionId] = {}
      }
      recoveredQuery[collectionId][viewId] =
        collectionData?.result?.reducerResults || {}

      if (collectionData?.recordMap) {
        mergedRecordMap = {
          block: {
            ...(mergedRecordMap?.block || {}),
            ...(collectionData.recordMap.block || {})
          },
          collection: {
            ...(mergedRecordMap?.collection || {}),
            ...(collectionData.recordMap.collection || {})
          },
          collection_view: {
            ...(mergedRecordMap?.collection_view || {}),
  ...
  return {
    pageIds: [...recoveredPageIds],
    recordMap: mergedRecordMap,
    collectionQuery: recoveredQuery
  }
}
...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  for (const table of ['Config-Table', 'CONFIG-TABLE']) {
    ...
    content =
      normalizePageBlock(pageRecordMap?.block?.[configPageId]?.value)?.content ||
      pageRecordMap?.block?.[configPageId]?.value?.content
  }
  ...
  let collectionView = pageRecordMap?.collection_view
  ...
  if (pageIds?.length === 0) {
    const recovered = await recoverConfigCollectionQuery({
      collectionId,
      collectionView,
      viewIds,
      spaceId: rawMetadata?.space_id || null
    })

    if (recovered?.pageIds?.length > 0) {
      pageIds = recovered.pageIds
      collectionQuery = {
        ...(pageRecordMap.collection_query || {}),
        ...(recovered.collectionQuery || {})
      }
      pageRecordMap.collection_query = collectionQuery
      collectionView = {
        ...(pageRecordMap.collection_view || {}),
        ...(recovered.recordMap?.collection_view || {})
      }
      pageRecordMap.collection_view = collectionView
      pageRecordMap.block = {
        ...(pageRecordMap.block || {}),
        ...(recovered.recordMap?.block || {})
      }
    }
  }
